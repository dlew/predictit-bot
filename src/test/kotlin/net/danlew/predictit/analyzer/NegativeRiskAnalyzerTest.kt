...
import net.danlew.predictit.test.createMemoryDb
...
class NegativeRiskAnalyzerTest {
  ...
  fun analyze() {
    ...
    val analyzer = NegativeRiskAnalyzer(createMemoryDb(), 0)

    // Close, but no cigar
    assertEquals(
      emptySet<Notification>(),
      analyzer.analyze(null, generateMarket(Price(20), Price(10), Price(20)))
    )

    assertEquals(
      emptySet<Notification>(),
      analyzer.analyze(null, generateMarket(Price(50), Price(10), Price(40)))
    )

    assertEquals(
      emptySet<Notification>(),
      analyzer.analyze(null, generateMarket(Price(90), Price(10), Price(10)))
    )

    // Yay negative risk
    assertEquals(
      setOf(expectedNotification),
      analyzer.analyze(null, generateMarket(Price(90), Price(10), Price(11)))
    )

    assertEquals(
      setOf(expectedNotification),
      analyzer.analyze(null, generateMarket(Price(75), Price(75), Price(75)))
    )
  }
  ...
  fun excludesOneCentContracts() {
    ...
    assertEquals(
      emptySet<Notification>(),
      analyzer.analyze(null, generateMarket(Price(90), Price(20), Price(1)))
    )
  }
  ...
  fun doesNotAnalyzeClosedMarkets() {
    ...
    assertEquals(
      emptySet<Notification>(),
      analyzer.analyze(null, closedMarket)
    )
  }
